...
_HEADER_KEYWORDS = [
    ...
    "transaction", "chq", "ref", "value date", "amount"
]
...
def _is_header_row(row: list[str | None], prev_header: list[str] | None = None) -> bool:
    ...
    if prev_header and len(row) == len(prev_header):
        similarity = sum(1 for a, b in zip(row, prev_header) if str(a or "").strip().lower() == str(b or "").strip().lower())
        if similarity / max(len(row), 1) > 0.7:
            return True
    return False


def _extract_grid_tables(pdf: pdfplumber.PDF) -> tuple[list[list[str]], Optional[list[str]]]:
    """Strategy A: Extracts structured grid tables across pages and preserves column headers."""
    all_rows: list[list[str]] = []
    main_header: Optional[list[str]] = None
    ...
    for page in pdf.pages:
        ...
        for row in data_rows:
            ...
            if main_header and [c.lower() for c in cleaned_row] == [c.lower() for c in main_header]:
                continue
...
def _extract_borderless_text(pdf: pdfplumber.PDF) -> tuple[list[list[str]], Optional[list[str]]]:
    ...
    for page in pdf.pages:
        ...
        for line in lines:
            ...
            if any(bp in lower_line for bp in ["statement generated", "page ", "crn ", "ifsc ", "micr ", "account statement", "branch "]):
                if not any(k in lower_line for k in ["installment", "sweep", "neft", "upi", "pos", "ecom", "pcd", "transfer"]):
                    continue
    ...
    header = ["Transaction Date", "Value Date", "Description", "Reference No", "Debit Amount", "Credit Amount", "Balance"]
    rows: list[list[str]] = []
...
def extract_pdf(file_path: str | Path) -> tuple[list[list[str]], Optional[list[str]], str]:
    """
    ...
    try:
        with pdfplumber.open(path) as pdf:
            ...
            if len(rows) >= 2 and header is not None and len(header) >= 3:
                logger.info("PDF extraction succeeded via Strategy A (Grid Tables): %d rows", len(rows))
            ...
            if len(rows) >= 2 and header is not None:
                logger.info("PDF extraction succeeded via Strategy B (Borderless Text): %d rows", len(rows))
