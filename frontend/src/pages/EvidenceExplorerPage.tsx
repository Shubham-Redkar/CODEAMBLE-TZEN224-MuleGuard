    ...
    <div className="p-6 max-w-5xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-500 text-xs mt-1">Full audit trail & deterministic proof bundle</p>
        </div>
        <div className="flex gap-2">
          <Link to={`/dashboard/${effectiveId}`} className="px-3 py-1.5 bg-white border rounded-lg text-xs font-medium hover:bg-gray-50 flex items-center gap-1.5 shadow-sm">
            <BarChart2 className="w-3.5 h-3.5" /> Dashboard
          </Link>
          <Link to={`/graph/${effectiveId}`} className="px-3 py-1.5 bg-white border rounded-lg text-xs font-medium hover:bg-gray-50 flex items-center gap-1.5 shadow-sm">
            <GitGraph className="w-3.5 h-3.5 text-purple-600" /> Proof Graph
          </Link>
          <button onClick={() => setRawJson(!rawJson)} className="px-3 py-1.5 bg-white border rounded-lg text-xs font-medium hover:bg-gray-50 flex items-center gap-1.5 shadow-sm">
            <FileText className="w-3.5 h-3.5 text-gray-600" /> {rawJson ? "Formatted View" : "Raw JSON"}
          </button>
          <button onClick={handleExport} className="px-3 py-1.5 bg-blue-600 text-white rounded-lg text-xs font-medium hover:bg-blue-700 flex items-center gap-1.5 shadow-sm">
